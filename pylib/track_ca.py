...
class MultiTracker:
    ...
    def _get_int_dtype(self):
        if self.bitwidth >= 64:
            return self.np.uint64
        if self.bitwidth >= 32:
            return self.np.uint32
        if self.bitwidth >= 16:
            return self.np.uint16
    ...
    def __init__(
        ...
        *,
        ...
        assert (
            dstream_S in self._supported_S
        ), f"dstream_S must be in {self._supported_S}"
    ...
    def initialize(self, data: np.ndarray, R: int):
        ...
        self.hst_markers = self.np.random.randint(
            0,
            2**self.bitwidth,
            size=(self.N, H, W, self.S),
            dtype=self.marker_dtype,
    ...
    def step(self, data, *, save: bool = False, add_random_noise: bool = False):
        ...
        scores = self.np.concatenate(ancestor_matrices)[None].repeat(
            self.N, axis=0
        )
        ...
        if add_random_noise:
            ...
            scores += self.rng.uniform(
                low=0, high=0.5, size=scores.shape
            ).astype(
                self.np.float16
            )  # 0.5 to ensure 0's don't beat 1's

        winning_scores = (
            self.parents.transpose(1, 2, 0)[None]
            .repeat(self.N, axis=0)[
                self.np.eye(scores.shape[1], dtype=self.np.bool_)[
        ...
        self.hst_markers[:] = self.hst_markers[
            self.np.arange(self.hst_markers.shape[0]).reshape(
                self.hst_markers.shape[0], 1, 1
            ),
    ...
    def reconstruct_phylogenies(self, *, verbose=False):
        ...
        for idx in range(self.N):
            extant_information = [
                (
                    ...
                    np.apply_along_axis(
                        self._pack_hex,
                        -1,
                        self.to_numpy(hst_markers[idx, ca_state]),
                    ),
                )
                    ...
                    population.append(
                        {
                            ...
                            "data_hex": np.asarray(t, dtype=">u4")
                            .tobytes()
                            .hex()
                            + i,
            ...
            postprocessor = AssignOriginTimeNodeRankTriePostprocessor(
                t0="dstream_S"
            )
            df = surface_unpack_reconstruct(
                pl.from_pandas(pd.DataFrame(population))
            ).to_pandas()
...
class CATracker(MultiTracker):
    def __init__(
        ...
        *,
        ...
        super().__init__(
            dstream_S, dstream_bitwidth, dstream_algo, N=1, backend=backend
        )
...
def track_ca_history(
    ...
    *,
    ...
    for i, curr in tqdm.tqdm(
        enumerate(data[1:], start=1), total=data.shape[0] - 1
    ):
        ca_tracker.step(
            curr, save=i in fossil_range, add_random_noise=add_random_noise
        )
...
def plot_phylo_at(
    ...
    grid_dim = (
        int(df[df["gol_state"] >= 0][["row", "col"]].to_numpy().max()) + 1
    )
    ...
    with tp.teed(
        ...
        for idx, row in df[df["is_leaf"] & df["extant"]].iterrows():
            ...
            tree_x, tree_y = next(
                v
                for n, v in tree_left.get_layout().T.items()
...
def plot_ancestry_at(
    phylo_df: pd.DataFrame,
    parent_rank: int,
    teeplot_subdir: str,
    child_rank: typing.Optional[int] = None,
    show: bool = False,
) -> None:
    ...
    dp_tree_for_plotting = apc.alife_dataframe_to_dendropy_trees(
        hstrat._auxiliary_lib.alifestd_try_add_ancestor_list_col(
            df_no_origin_time
        ),
        setup_edge_lengths=True,
    )
        ...
        max(
            (
                (
                    parent_id,
                    df_by_id.at[
                        pdm.mrca(child_id, parent_id).taxon.label,
                        "hstrat_rank",
                    ],
                )
                for parent_id in dp_tree[0].taxon_namespace
                if parent_id.label in parent_taxa
            ),
            key=lambda x: x[1],
        )[0].label
        for child_id in dp_tree[0].taxon_namespace
        if child_id.label in child_taxa
    ]
    ...
    with tp.teed(
        ...
        for i, (rank, ax) in enumerate(
            [(parent_rank, ax_left), (child_rank, ax_right)]
        ):
            ...
            sns.heatmap(
                ...
                vmax=1,
                cmap=ListedColormap(
                    sns.color_palette(["#0099ff" if i == 1 else "#ff0000"])
                ),
                cbar=False,
            )
            # ax.set_axis_off()
            ...
            for _, row in phylo_df[
                ...
                tree_x, tree_y = next(
                    v
                    for n, v in tree.get_layout().T.items()
                    if n.taxon is not None and n.taxon.label == row["id"]
                )
                grid_x, grid_y = row["col"], row["row"]
        ...
        for child, parent in zip(child_taxa, ancestors):
            child_y, child_x = (
                df_by_id.at[child, "row"],
                df_by_id.at[child, "col"],
            )
            parent_y, parent_x = (
                df_by_id.at[parent, "row"],
                df_by_id.at[parent, "col"],
            )
            con = ConnectionPatch(
