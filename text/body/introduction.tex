...
Such limitations hinder use of agent-based simulation methods for studying phenomena that only emerge across simulation scales.
...
Likewise, simulation scale has been highlighted in studying evolutionary transitions in individuality, evo-ecological dynamics, and rare evolutionary innovations \citep{taylor2016open,dolson2021digital,taylor2019evolutionary} --- key questions, in particular, to the topic of open-ended evolution in artificial life research \citep{TODO}.
As a bioinformatics testbed, identifying the effects of sample size on sensitivity and accuracy . to motivate large sample sizes being a question \citep{moreno2025extending,TODO}, which can be a key first step to motivating collection of expensive real-world datasets.

Fortunately, evolution simulations are generally well-suited to accelerator platforms.
Given their underlying stochastic nature, they are generally robust to lower-precision computations.
Likewise, population structure in space is helpful to dealing with the highly-distributed nature of accelerator processors.
Finally, data collection is often robust to sampling-based approaches --- as this is typically necessary in \textit{in vivo} experiments.
In the above examples, though, phylogeny data are crucial --- MODES \citep{dolsonMODESToolboxMeasurements2019,TODO}.
motivating the work developed here.
Before getting to the topic of how phylogenies might be collected large-scale parallel and distributed evolution simulations, let's first review typical direct tracking approaches to phylogeny.
