...
% Key challlenges exist in data collection, memory constraints on data collection.
...
As further discussed below, where lineages traverse across distributed memory spaces these ancestry records can accumulate a substantial memory footprint --- even with additional coordination mechanisms to weed out extinct lineages.

Given the key role of phylogeny data for informative evolution simulations --- and effective application-oriented evolutionary computation --- the cost and complexity of exact ancestry tracking poses a major obstacle to fully harnessing next-generation AI/ML hardware accelerators.
...
Because work with natural biosequence data is notoriously data- and compute-intensive, our focus is in developing a structure for synthetic genome data to allow fast reconstructions from small sequences (e.g., as little as 64 bits).
Given that requirements for inference will differ substantially by both underlying evolutionary conditions and overarching analytical/experimental objectives, we ensure that memory use may be tuned to desired inference accuracy and that emphasis in resolution power may be adjusted between recent and ancient history.
