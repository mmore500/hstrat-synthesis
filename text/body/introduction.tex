...
% as well as underlying evolutioneary conditions, and overarching analytical/experimental objectives
...
\unskip\footnote{%
This small scaling coefficient arises a result of coalescence: the statistical tendency for an extant population's ancestry to merge back into just a few persisting lines from the distant past \citep{TODO}.
}
...
As expected, distributed memory footprints for direct tracking above are substantially greater than the 12kB required to store timestamped unique identfiers for direct tracking in a unified memory space.
% $2N$
...
Instead, it would instead be advantageous in many circumstances to trade a measured amount of accuracy and precision in phylogenetic inference for improved resource efficiency.
...
% To some degree,
% In memory-constrained environments, therefore, these require substinative decreases in population size.
...
For systems with non-uniform I/O access, such as the WSE's periphery-only host-device connectivity, data collection can even be concentrated where it is most readily accomplished --- provided downstream analyses can accomodate associated sampling biases.
...
In a production environment, compute node failure could sever history of all lineages emigranted from that node.
...
Although ancestry information can be extracted directly from the content of digital genome models \citep{TODOOEE4}, this approach won't work well for small genomes, requires case-by-case specific modifications (doesn't generalize across models), and subject to bias from selection effects, and encounters challenges in reconstruction efficiency typical of work with biological genomes.
