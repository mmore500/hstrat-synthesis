...
Indeed, longstanding and productive exchange has existed between evolution \textit{in silico} and \textit{in vivo} \textit{pennock2007models,dolson2021digital}.
While experimental approaches to the study of evolution have gained substantial traction \citep{TODO}, simulation approaches have brought to the fore study designs infeasible to observe on human timescales or otherwise difficult to engineer \citep{wiser2013long,Stroud2025}.
...
Although serial simulation code can achieve rapid generational turnover and high replicate count, typically this is at the cost of small population sizes --- especially when working with rich agent models \citep{TODOdissertation}.
...
Likewise, limitations of simulation scale have been highlighted in studying evolutionary transitions in individuality, evo-ecological dynamics, and rare evolutionary innovations \citep{taylor2016open,dolson2021digital,taylor2019evolutionary} --- key questions, in particular, to the topic of open-ended evolution in artificial life research \citep{TODOackley}.
In the bioinformatics testbed domain, limitations of scale can obscure the relationships of population size and sampling density with sensitivity and accuracy \citep{moreno2025extending,TODO}.
...
Given fundamental underlying stochasticity in evolution, these simulations are generally robust to lower-precision computations.
Similarly, since many natural populations are spatially structured, on-chip data flow constraints can be mitigated by aligning with locality in simulation structure.
Finally, data collection can often tolerate sampling or coarsening, much as \textit{in vivo} data collection is itself often incomplete or sampled.

However, this general suitability for accelerators does not easily extend to collecting phylogeny data, which is a critical ingredient both to conducting informative large-scale evolution experiments --- as in examples highlighted above --- and to orchestrating effective accelerator-friendly evolutionary computation.
For instance, in the context of open-ended evolution, phylogeny data has been highlighted as a core metric in measuring ongoing system change \citep{dolsonMODESToolboxMeasurements2019}.
It is the need to provide this critical capability in large-scale systems that motivates work developed here.
Before introducing our proposed approach, though, a review of phylogeny tracking typically used in serial simulation is first warranted.
...
Given the utility of , tracking phylogenetic trees is a common practice in simulation experiments \citep{ray1992evolution,bohm2017mabe,de2012deap,garwood2019revosim,godin2019apoget,dolson2024phylotrackpy,haller2023slim,Guillaume2006,OFallon2010,Thornton2014,okamoto2017framework,Vlachos2018,schumaker2018hexsim,MattheyDoret2021}.
Typical practice to record lineage histories is to accrete every parent-child relationship as it occurs to create a comprehensive tree data structure \citep{moreno2024algorithms} (Figure \ref{fig:tracking-vs-reconstruction-schematic:tracking}).
...
Solving this problem is nontrivial, and we describe a solution on it here.
